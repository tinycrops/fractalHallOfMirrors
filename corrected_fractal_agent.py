...
def train_agent():
    ...
    for ep in trange(episodes):
        ...
        high_level_decisions = 0  # Optional: track number of high-level decisions
        ...
        for t in range(horizon):
            ...
            high_level_decisions += 1  # Increment high-level decision counter
            ...
            while steps_within_super < max_steps_super and not super_done and not done:
                ...
                for _ in range(block_micro*block_micro):
                    ...
                    primitive_steps += 1  # COUNT EVERY PRIMITIVE ACTION
                    total_micro_steps += 1
...
def visualize_agent():
    ...
    primitive_steps = 0  # Count primitive steps during visualization
    ...
    while True:
        ...
        pos, _, _ = step(pos, a)
        primitive_steps += 1  # Count primitive steps during visualization
        path.append(pos)

    print(f"Visualization path completed in {primitive_steps} primitive steps")
...
def plot_learning_curve(log):
    ...
    plt.subplot(2, 1, 1)
    plt.plot(log, '-o', alpha=0.4, markersize=2, label='Primitive Steps per Episode')
    ...
    plt.xlabel('Episode')
    plt.ylabel('Primitive Steps to Goal')
    ...
    plt.xlabel('Episode')
    plt.ylabel('Primitive Steps to Goal (log scale)')
    plt.grid(True, alpha=0.3)
    plt.legend()
...
def main():
    print("Training the corrected three-level fractal Q-learning agent...")
    print("Now properly counting EVERY primitive action (micro-step)")
    log, training_time = train_agent()
    
    print("\nTraining complete!")
    print(f"Final performance: {log[-1]} primitive steps to goal")
    print(f"Best performance: {min(log)} primitive steps to goal")
    print(f"Training time: {training_time:.2f} seconds")
    
    # Save log data for comparison
    np.save('corrected_fractal_agent_log.npy', np.array(log))
