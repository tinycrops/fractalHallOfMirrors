#!/usr/bin/env python3
"""
Educational Fractal Self-Observation Visualization
...
The visualizations are designed to be intuitive and educational, suitable
for demonstrating the core concepts of fractal self-observation.
"""
...
• Cross-scale learning improvements
    ...
    def create_step_by_step_animation(self, agent_trajectory, save_path=None, interval=1000):
        """Create an animated step-by-step visualization of the learning process."""
        fig = plt.figure(figsize=(20, 16))
        
        def animate(frame):
            fig.clear()
            
            # Recreate the educational overview for this frame
            current_step = frame
            
            # Temporarily set trajectory for this frame
            old_trajectory = self.full_trajectory.copy()
            self.full_trajectory = agent_trajectory[:frame+1] if frame < len(agent_trajectory) else agent_trajectory
            
            # Create the visualization
            self.create_educational_overview(current_step=current_step, show_explanations=(frame == 0))
            
            # Restore original trajectory
            self.full_trajectory = old_trajectory
            
            return []
        
        anim = FuncAnimation(fig, animate, frames=len(agent_trajectory), 
    ...
    def create_learning_summary(self):
        """Create a comprehensive learning summary visualization."""
        if not self.full_trajectory:
            print("No trajectory data available for summary")
            return None
        
        fig, axes = plt.subplots(2, 3, figsize=(18, 12))
        fig.suptitle('Fractal Self-Observation Learning Summary', fontsize=16, fontweight='bold')
        
        # 1. Exploration heatmap
        ax = axes[0, 0]
        positions = [(step['state'][0], step['state'][1]) for step in self.full_trajectory]
        ...
        # 2. Depth usage over time
        ax = axes[0, 1]
        steps = [step['step'] for step in self.full_trajectory]
        depths = [step['state'][2] for step in self.full_trajectory]
        ax.plot(steps, depths, linewidth=2, alpha=0.8)
        ax.set_title('Depth Exploration Over Time')
        ax.set_xlabel('Step')
        ...
        rewards = [step['reward'] for step in self.full_trajectory]
        cumulative_rewards = np.cumsum(rewards)
        ax.plot(steps, cumulative_rewards, color='green', linewidth=2)
        ...
        ax = axes[1, 0]
        if self.depth_transitions:
            transition_types = [t['type'] for t in self.depth_transitions]
            type_counts = {t: transition_types.count(t) for t in set(transition_types)}
            ax.bar(type_counts.keys(), type_counts.values(), color=['blue', 'red'])
            ax.set_title('Depth Transition Types')
            ax.set_ylabel('Count')
        else:
            ax.text(0.5, 0.5, 'No depth transitions', ha='center', va='center', transform=ax.transAxes)
            ax.set_title('Depth Transition Types')
        
        # 5. Self-observation development
        ax = axes[1, 1]
        if self.self_observation_events:
            obs_steps = [event['step'] for event in self.self_observation_events]
            obs_scales = [event['observed_scales'] for event in self.self_observation_events]
            ax.scatter(obs_steps, obs_scales, alpha=0.7, s=50)
            ax.set_title('Self-Observation Development')
            ax.set_xlabel('Step')
            ax.set_ylabel('Scales Observed')
            ax.grid(True, alpha=0.3)
        else:
            ax.text(0.5, 0.5, 'No self-observation\nevents recorded', 
...
Total Steps: {len(self.full_trajectory)}
Depth Transitions: {len(self.depth_transitions)}
Max Depth Reached: {max(step['state'][2] for step in self.full_trajectory)}
Self-Observation Events: {len(self.self_observation_events)}
Final Reward: {cumulative_rewards[-1]:.2f}

FRACTAL CAPABILITIES:
• Multi-scale navigation: {'✓' if self.depth_transitions else '✗'}
• Self-observation: {'✓' if self.self_observation_events else '✗'}
• Cross-scale learning: {'✓' if len(set(depths)) > 1 else '✗'}
        """
...
def run_educational_demonstration():
    ...
    fig2 = visualizer.create_learning_summary()
    plt.show()
    ...
    if len(visualizer.depth_transitions) > 10:
        print(f"\n🎯 SUCCESS: Agent developed fractal navigation capabilities!")
        if len(visualizer.self_observation_events) > 5:
...
if __name__ == "__main__":
    ...
    # Optionally create an animation (commented out for performance)
    # print("\nCreating step-by-step animation...")
    # anim = visualizer.create_step_by_step_animation(trajectory[:50], save_path="fractal_learning.gif")
    # plt.show() 